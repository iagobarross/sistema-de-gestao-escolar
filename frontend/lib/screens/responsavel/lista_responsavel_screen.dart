...
class _ListaResponsavelScreenState extends State<ListaResponsavelScreen> {
  ...
  Future<void> _navegarParaFormulario({Responsavel? responsavel}) async {
    final bool? resultado = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => FormResponsavelScreen(responsavelParaEditar: responsavel),
      ),
    );
    if (resultado == true) {
      _carregarResponsaveis();
  ...
  Future<void> _deletarResponsavel(int id) async {
    ...
    if (confirmou) {
      ...
      try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Responsável excluído com sucesso!')));
        _carregarResponsaveis();
      } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [IconButton(icon: Icon(Icons.refresh), onPressed: _carregarResponsaveis)],
      ),
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  subtitle: Text("CPF: ${responsavel.cpf} | Telefone: ${responsavel.telefone}"),
                  trailing: IconButton(
                    icon: Icon(Icons.delete_outline, color: Colors.red),
                    onPressed: () => _deletarResponsavel(responsavel.id),
                  ),
                  onTap: () => _navegarParaFormulario(responsavel: responsavel),
                );
              },
            );
          } else {
